...
def train_matrix():
    ...
    ic(
        "\n {0} фотографий прошли обучение. Выход из программы".format(
            len(np.unique(ids))
        )
    )
...
def save_user(message, user_data: dict[str, Any]):
    ...
    ic(face_id)
    bot.send_message(message.from_user.id, "Посмотрите в камеру", parse_mode="Markdown")
    # time.sleep(2000)
    create_user_dataset(face_id)
    train_matrix()
    bot.send_message(message.from_user.id, "Вы зарегистрированы", parse_mode="Markdown")
    # bot.send_message(message.from_user.id, "Введите класс", parse_mode='Markdown')
    # bot.register_next_step_handler(message, save_user, user_data=user_data)
...
def save_user_settings(user_settings=dict[str, Any]) -> None:
    ic(user_settings)
    # FIXME: Соединение должно открываться на старте бота, закрываться при его остановке, а в функцию попадать параметром
    with sqlite3.connect("database_people.db") as connection:
        connection.row_factory = sqlite3.Row  # Enable dictionary-like access
        cursor = connection.cursor()
        a = cursor.execute(
        ...
        # print(a)
        connection.commit()
        assert cursor.rowcount == 1, f"Expected 1 modified row, not {cursor.rowcount}"
        cursor.close()
...
if __name__ == "__main__":
    ...
    # telegram_id = "".join(random.choices(string.digits, k=10))
    # create_user(
    #     {
    #         "first_name": "Ололох",
    #         "last_name": "Ололоев",
    #         "patronymic_name": "Ололоевич",
    #         "grade": 10,
    #         "class_number": 2,
    #         "telegram_id": telegram_id,
    #     }
